...
## 7. Predictive Appearance Model: DINOv3 Foundation Features & Sensitometric Inversion

### 7.1 Limitations of Empirical 3D cLUT Profiling

The conventional negative film inversion pipeline implemented in Sections 4 and 5 maps linear sensor transmittances to target positive colorspaces via discrete 3D color lookup tables (cLUTs) generated by ArgyllCMS `colprof` from IT8 calibration targets. While mathematically exact for the specific laboratory exposures of the target, this approach introduces three fundamental limitations when applied to general photography:

1. **Scene Semantic Blindness**: An IT8 profile is strictly a physical patch transfer function. It contains zero contextual awareness regarding scene lighting (e.g. golden hour vs. overcast noon vs. indoor tungsten) or subject matter (e.g. human skin tones vs. foliage vs. architecture).
2. **Discrete Target Bracketing**: Scanned negatives rarely align exactly with the discrete exposure brackets (Target 1 through Target 5) captured during calibration. Selecting a discrete target requires heuristic mid-grey distance matching, which can cause jarring color shifts across a roll.
3. **Severe Boundary Extrapolation**: Under-exposed (thin) or over-exposed (dense) negatives fall outside the convex hull of the calibration patches. Fixed 3D cLUT tetrahedral interpolation clips these values, producing severe color artifacts, blocked shadows, or blown-out highlights.

To overcome these constraints, the station incorporates an experimental **Predictive Appearance Model** implemented in [`src/appearance_model_film_conversion.py`](src/appearance_model_film_conversion.py). It unifies classical **sensitometric Hurter & Driffield (H&D)** negative film physics with **DINOv3 vision foundation representations**.

---

### 7.2 Physical Sensitometric Modeling: Transmittance and Optical Density

Color negative film is a subtractive imaging medium consisting of three emulsion layers sensitized to blue, green, and red light. During development, oxidized color developer couples with dye couplers to produce yellow, magenta, and cyan dye clouds, alongside an unexposed orange masking layer (the film base).

#### 1. Sensor Decoupling and Physical Transmittance
Raw sensor responses $I_{\text{sensor}} \in \mathbb{R}^{3}$ are first decoupled from camera color filter array (CFA) crosstalk using the calibrated $3 \times 3$ crosstalk matrix $M_{\text{cfa}}$:
$$I_{\text{cfa}} = I_{\text{sensor}} \cdot M_{\text{cfa}}^T$$

The unexposed film base $\mathbf{base}_{\text{cfa}}$ is measured at calibration exposure ($t_{\text{base}}, \text{iso}_{\text{base}}$). When a negative is scanned at arbitrary scanner shutter speed $t_{\text{scan}}$ and sensitivity $\text{iso}_{\text{scan}}$, the exposure ratio between base and scan is determined via the shared utility function [`compute_exposure_ratio`](src/film_profiling.py):
$$E_{\text{base}} = t_{\text{base}} \times \left(\frac{\text{iso}_{\text{base}}}{100.0}\right), \quad E_{\text{scan}} = t_{\text{scan}} \times \left(\frac{\text{iso}_{\text{scan}}}{100.0}\right)$$
$$\text{exposure\_ratio} = \frac{E_{\text{base}}}{E_{\text{scan}}}$$
$$\mathbf{base}_{\text{aligned}} = \frac{\mathbf{base}_{\text{cfa}}}{\text{exposure\_ratio}}$$

#### 2. Highlight Safeguard and Optical Density
To guarantee physical admissibility ($T \le 1.0$, $D \ge 0.0$) across unevenly illuminated film gates or thin negative borders, an effective film base ceiling is applied:
$$\mathbf{base}_{\text{eff}} = \max\left(\mathbf{base}_{\text{aligned}}, \text{percentile}_{99.8}(I_{\text{cfa}})\right)$$

Spectral transmittance $T$ and optical density $D$ are then computed per channel:
$$T = \text{clip}\left(\frac{I_{\text{cfa}}}{\mathbf{base}_{\text{eff}} + \epsilon}, 10^{-4}, 1.0\right)$$
$$D = -\log_{10}(T)$$

Because optical density is logarithmic, subtractive dye absorption is strictly additive in density space:
$$D_c = D_{\text{base}, c} + D_{\text{dye}, c} \quad \text{for } c \in \{R, G, B\}$$

Subtracting the base density $D_{\min} = \text{percentile}_{0.2}(D)$ removes the orange mask completely in a linear, physically authentic manner without dynamic range compression.

---

### 7.3 Two-Stage Cascade Inversion Architecture

Direct end-to-end neural regression of film negatives frequently fails due to huge spectral variance and training instability. The predictive appearance model employs a robust **two-stage cascade**:

```
 RAW Image (.ARW)
        │
        ▼ (negicc_station LibRaw debayer)
 Linear CFA RGB
        │
        ▼ (crosstalk_calibration M_cfa)
 Optical Density D = -log10(T)
        │
 ┌──────┴────────────────────────────────────────┐
 │ Stage 1: Pass 0 Bootstrap (Preview Hillclimb)  │
 │ 1D parametric curve alignment (Photoshop Auto)│
 └──────┬────────────────────────────────────────┘
        │
        ▼ Normalized Preview RGB
 ┌──────┴────────────────────────────────────────┐
 │ Stage 2: Physical Sensitometric Fixed-Point   │
 │ • DINOv3 Small ViT (Layers 9 + 12 tokens)     │
 │ • Photographic Intent Adapter (mu, sigma)     │
 │ • Hurter & Driffield Physical Curve Inversion │
 └──────┬────────────────────────────────────────┘
        │
        ▼
 Positive RGB Image (.jpg / .tiff)
```

#### Stage 1: Pass 0 Preview Bootstrap
Stage 1 performs an extremely fast ($<3.5$s on CPU) 1D parametric curve optimization initialized by luminance percentile balancing. It models the empirical response of standard photo-lab contrast curves:
$$\mathbf{y}_{\text{preview}} = f_{\text{curve}}(D; \theta_{\text{bootstrap}})$$
This produces an upright, properly exposed preview image that neutralizes extreme color casts and exposes scene geometry to the vision backbone.

#### Stage 2: DINOv3 Intent Field & Fixed-Point Inversion
Stage 2 feeds the normalized preview $\mathbf{y}_{\text{preview}}$ into a frozen **DINOv3 Small** vision transformer backbone (300M parameters, patch size $16\times 16$).
1. **Multi-Layer Representation**: Features are extracted from both intermediate Layer 9 (capturing mid-level textures, chrominance boundaries, and local contrast) and Layer 12 (capturing high-level semantic priors such as skin tones, skies, and foliage).
2. **Photographic Intent Adapter**: A lightweight adapter projects the fused DINOv3 token features into spatial continuous target CIELAB distributions:
   $$\mu_L, \mu_a, \mu_b, \sigma_L, \sigma_a, \sigma_b = \text{Adapter}\left(\mathbf{F}_{\text{layer9}} \oplus \mathbf{F}_{\text{layer12}}\right)$$
3. **Physical Fixed-Point Solver**:
   The physical inversion maps optical density $D$ back to scene exposure $E_{\text{scene}}$ via the Hurter & Driffield relation:
   $$I_{\text{pos}}(c) = g_c \cdot 10^{-\frac{D(c) - D_{\min}(c)}{\gamma_c}}$$
   where $\gamma = (\gamma_R, \gamma_G, \gamma_B)$ represents the individual dye layer contrast gammas and $g = (g_R, g_G, g_B)$ represents channel gains.
   
   The solver minimizes the perceptual CIEDE2000 difference between the inverted positive and the predicted intent field:
   $$\mathcal{L} = \Delta E_{00}\left(\text{CIELAB}(I_{\text{pos}}), \mathbf{\mu}_{\text{intent}}\right) + \lambda_{\text{smooth}} \|\nabla \gamma\|^2$$

Within 2–5 fixed-point iterations ($\sim 10$s total CPU time), the physical parameters converge to $\Delta E_{00} < 1.0$, producing natural, grain-preserving, film-accurate positive images.

---

### 7.4 Architectural Advantages over 3D cLUTs

| Characteristic | Profiled Method (ArgyllCMS 3D cLUT) | Predictive Appearance Model (DINOv3 + Sensitometric) |
| :--- | :--- | :--- |
| **Input Domain** | Fixed IT8 bracket transmittances | Physical optical density $D = -\log_{10}(T)$ |
| **Scene Awareness** | None (pure 1:1 color transfer) | Full semantic context (DINOv3 vision foundation) |
| **Continuity** | Discretized tetrahedral grid ($33^3$ or $65^3$) | Infinite continuous parameter space |
| **Exposure Invariance** | Requires target selection heuristic | Invariant via physical `compute_exposure_ratio` alignment |
| **Skin/Sky Tone Balance** | Fixed to calibration patch balance | Adaptive photographic intent guidance |
| **Quantization Artifacts** | Subject to 16-bit CMM rounding noise | Zero quantization (pure float32 mathematical solver) |
| **Execution Speed** | $\sim 0.6$s (CPU) / $0.13$s (CUDA) | $\sim 19$s (CPU single-shot inference) |
