...
![Sample Scan Overview (Portra 400, +2 stops overexposure)](test_imgs/sample_corrected_portra400.jpg)
*Sample scan overview (Kodak Portra 400, +2 stops overexposure).*
...
![Sample Scan (Portra 400, +2 stops overexposure)](test_imgs/Portra400_corrected_0007.jpg)
*Sample scan (Kodak Portra 400, +2 stops overexposure).*
...
Even with a tricolor or narrow-band light source, physical channel separation is not enough. Without profiling the specific film stock, the captured RGB values remain arbitrary and lack color space metadata.
* **Sensor Ubiquity**: A tricolor light source is not strictly required. True monochrome sensors are rare; standard color filter arrays (CFAs) with mathematical correction is the most common and robust approach in digital image processing. Using pixel-shift captures (e.g., on the Sony A7R4) combined with a crosstalk correction matrix achieves the same channel purity as physical separation, utilizing readily available hardware.
* **Colorspace Mapping**: After crosstalk correction and film base normalization, the linear RGB values must be mapped to a known color space, otherwise they are just arbitrary values. This project uses a profiled approach to map these linear values to absolute XYZ coordinates.
* **Exposure & ISO Latitude**: This mapping allows the system to automatically handle varying exposure levels (negatives that are too dense or too thin) and scanning ISO sensitivities.
* **Non-linear Dye Gamma Curves**: Crucially, the cyan, magenta, and yellow dyes in negative film emulsions do not share the same gamma curves. A profiled approach fits independent 1D Tone Reproduction Curves (TRCs) for each channel to model these non-linearities, producing accurate, lifelike results automatically without requiring manual post-processing adjustments.
